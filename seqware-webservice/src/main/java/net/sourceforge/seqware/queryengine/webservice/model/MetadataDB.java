...
public class MetadataDB {
     ...
     * 
     * private static SessionFactory buildSessionFactory() { try { // Create the SessionFactory from hibernate.cfg.xml return new
     * Configuration().configure("hibernate.properties").buildSessionFactory(); } catch (Throwable ex) { // Make sure you log the exception,
     * as it might be swallowed System.err.println("Initial SessionFactory creation failed." + ex); throw new
    ...
    private Connection setupConnection() {
        ...
        try {
            ...
            c = DriverManager.getConnection("jdbc:postgresql://" + server + "/" + db, user, pass);
    ...
    public ArrayList<Map<String, String>> getWorkflowParamMetadata(Integer id) {
        return (moreGenericGetMetadata("SELECT type, key, display, file_meta_type, default_value FROM workflow_param where workflow_id = "
                + id));
    }
    ...
    public HashMap<String, Object> getWorkflowRunInfo(int workflowRunAccession) {
        ...
        try {
            s = c.createStatement();
            ResultSet rs = null;
            rs = s.executeQuery("select workflow_run_id, status, create_tstmp, update_tstmp, host from workflow_run where sw_accession = "
            ...
            rs = s.executeQuery("select sw_accession, processing_id, algorithm, status from processing where ancestor_workflow_run_id = "
                    + workflowRunId + " or workflow_run_id = " + workflowRunId);
            while (rs.next()) {
                ...
                ResultSet rs2 = s2
                        .executeQuery("select sw_accession, file_path from file where file_id in (select file_id from processing_files where processing_id = "
                                + procId + ")");
                while (rs2.next()) {
    ...
    public Map getMetadata(long swid) {
        return getMetadata(
                swid,
                "SELECT f.file_path, p.description, p.sw_accession, p.parameters, p.create_tstmp, f.meta_type FROM processing as p, file as f, processing_files as pf "
                        + "WHERE f.file_id = pf.file_id AND p.processing_id = pf.processing_id AND p.sw_accession = "
                        + swid
                        + " AND "
    ...
    public Map getTagMetadata(long swid) {
        return getMetadata(
                ...
                "SELECT f.file_path, p.description, p.sw_accession, p.parameters, p.create_tstmp, f.meta_type FROM processing as p, file as f, processing_files as pf "
                        + "WHERE f.file_id = pf.file_id AND p.processing_id = pf.processing_id AND p.sw_accession = "
                        + swid
                        + " AND (f.meta_type = 'application/seqware-qe-tags' OR f.meta_type = 'application/seqware-qe-postgresql-db'"
    ...
    private Map getMetadata(long swid, String queryStr) {
        ...
        try {
            ...
            System.out.println("We got an exception while executing our query:" + "that probably means our SQL is invalid: " + query);
            se.printStackTrace();
            return (null);
        }
    ...
    public void addWorkflowRunParam(int workflowRunAccession, String key, String value, String type) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                s.execute("insert into workflow_run_param (workflow_run_id, key, value, type) values (" + workflowRunId + ", '" + key
                        + "', '" + value + "','" + type + "' )");
     ...
     * @return a int.
     */
    public int scheduleWorkflowRun(int workflowAccession, String name, String cwd, String iniContents, String command,
            String workflowTemplate, String status) {
     ...
     *            a {@link java.lang.String} object.
     */
    public void updateWorkflowRun(int workflowRunAccession, String name, String cwd, String iniContents, String command,
            String workflowTemplate, WorkflowRunStatus status) {
        Connection c = setupConnection();
        Statement s = null;
        ...
        try {
            s.execute("update workflow_run " + " set name = '" + name + "', current_working_dir = " + "'" + cwd + "', ini_file = '"
                    + iniContents + "', cmd = '" + command + "', workflow_template = '" + workflowTemplate + "', " + "status = '" + status
                    + "' where sw_accession = " + workflowRunAccession);
        } catch (SQLException se) {
            System.out.println("We got an exception while executing our query:" + "that probably means our SQL is invalid: ");
            se.printStackTrace();
        }
