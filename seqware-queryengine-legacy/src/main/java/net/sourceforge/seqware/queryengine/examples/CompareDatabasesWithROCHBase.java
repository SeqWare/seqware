...
/**
 * <p>CompareDatabasesWithROCHBase class.</p>
 *
 * @author boconnor
 *
 * Purpose:
 *
 * This is a hacked version of the previous example program to try to
 * minimize the number of queries to the HBase server.
 *
 ...
 * @version $Id: $Id
 */
public class CompareDatabasesWithROCHBase {
  ...
  public static void main(String[] args) {
    ...
    if (args.length < 26) {
      System.out.println("CompareDatabasesWithROC <db_dir_gold_standard> <db_dir_unknown> <cache_size_gold_standard> <cache_size_unknown> <locks_gold_standard>" +
          ...
          "<backend_unknown_type_[BerkeleyDB|HBase]> <unknown_genome_id> <unknown_reference_genome_id> <contig_strings_comma_sep>"+
    ...
    try {
      ...
      String[] contigsArr = args[24].split(",");
      String summaryOutputFile = args[25];
      
      // keep track of the variant IDs already processed
      HashMap<String, String> processed = new HashMap<String, String>();
      
      // keep track of the known good variants
      HashMap<String, HashMap<Integer, Variant>> knownGood = new HashMap<String, HashMap<Integer, Variant>>();
      
      // keep track of the unkown variants
      HashMap<String, HashMap<Integer, ArrayList<Variant>>> unknownInRegions = new HashMap<String, HashMap<Integer, ArrayList<Variant>>>();
      ...
      if (storeA != null && storeB != null) {
        ...
        // FIXME: iterate over chr here and make sure I clean out the shared hashmaps! I should be able to just check if the chr changes.
        ...
        while (itA.hasNext()) {
          ...
          if (var.getReadCount() >= minCoverageA && var.getReadCount() <= maxCoverageA && 
            ...
            HashMap<Integer, Variant> knownGoodOnChr = knownGood.get(var.getContig());
            if (knownGoodOnChr == null) { knownGoodOnChr = new HashMap<Integer, Variant>(); knownGood.put(var.getContig(), knownGoodOnChr); }
            knownGoodOnChr.put(new Integer(var.getStartPosition()), var);
            ...
            while(itB.hasNext()) {
              ...
              if (varB.getType() == Variant.SNV && !processed.containsKey(id)) {
                ...
                // maybe add to unknownInRegions
                HashMap<Integer, ArrayList<Variant>> unknowns = unknownInRegions.get(varB.getContig());
                if (unknowns == null) { unknowns = new HashMap<Integer, ArrayList<Variant>>(); unknownInRegions.put(var.getContig(), unknowns); }
                ArrayList<Variant> unknownVs = unknowns.get(start);
                if (unknownVs == null) { unknownVs = new ArrayList<Variant>(); unknowns.put(start, unknownVs); }
                unknownVs.add(varB);
                ...
                /*
                 * COMPLETELY REPLACE THIS WITH A LOOKUP IN THE HASH
                ...
                itSpecificA.close();
                */
                // lookup from hash rather than another query
                // HashMap<String, HashMap<Integer, Variant>> knownGood 
                HashMap<Integer, Variant> aVariants = knownGood.get(varB.getContig());
                
                if (aVariants != null) {
                  Variant specificA = aVariants.get(varB.getStartPosition());
                  if (specificA != null) {
                    log("      Looking back for variant from known good: "+specificA.getContig()+":"+specificA.getStartPosition()+"-"+specificA.getStopPosition()+" ref->var: "+specificA.getReferenceBase()+" -> "+specificA.getCalledBase());
                    // FIXME: this is a hack, I think I'm storing the venter variants in the wrong field!!!!
                    if ((specificA.getCalledBase().equals(varB.getCalledBase()) && specificA.getReferenceBase().equals(varB.getReferenceBase())) || (specificA.getCalledBase().equals(varB.getReferenceBase()) && specificA.getReferenceBase().equals(varB.getCalledBase()))) { 
