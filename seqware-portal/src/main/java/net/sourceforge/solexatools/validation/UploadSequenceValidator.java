...
public class UploadSequenceValidator extends UploadFileValidator implements Validator {
     ...
     * @param fileTypeId
     */
    public void validateExtension(MultipartFile fileOne, MultipartFile fileTwo, Integer fileTypeId, Errors errors) {
        ...
        if (!errors.hasErrors()) {
            ...
            if (!trueExtension.equals(firstFileExtension)) {
                errors.rejectValue("fileOne", "error.upload.bad.first.file.extension", new Object[] { trueExtension }, "bad extension");
            }
     ...
     * 
     * String firstFileName = fileOne.getOriginalFilename(); if(getFileService().isExists(firstFileName, folderStore)){
     * errors.rejectValue("fileOne", "error.upload.first.file.used"); }
     * 
     * if(fileTwo != null && !fileTwo.isEmpty()){ String secondFileName = fileTwo.getOriginalFilename();
     * if(getFileService().isExists(secondFileName, folderStore)){ errors.rejectValue("fileTwo", "error.upload.second.file.used"); } } }
     */
     ...
     * 
     * String firstFileName = fileOne.getOriginalFilename(); String secondFileName = fileTwo.getOriginalFilename();
     * 
     * if (errors.getFieldError("fileOne") == null && errors.getFieldError("fileTwo") == null) { // DONT END
     * if(firstFileName.trim().equals(secondFileName.trim())){ errors.reject("error.file.names.dont.match"); } } } }
     */
    /**
     * <p>
