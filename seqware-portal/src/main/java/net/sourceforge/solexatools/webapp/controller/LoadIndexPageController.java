...
public class LoadIndexPageController extends BaseCommandController {
    ...
    /**
     * <p>
     * Constructor for LoadIndexPageController.
     * </p>
     */
    ...
    private void addNewPageToTop(HttpServletRequest request, String newPage) {
        ...
        Log.info("Start. Number curr page = " + numberCurrentPage + "; List page size = " + listSavedPage.size());
        ...
        Log.info("End. Number curr page = " + numberCurrentPage + "; List page size = " + listSavedPage.size());
    }
    ...
    private boolean isEndPage(HttpServletRequest request) {
        return (getNumberCurrentPage(request) == getListSavedPage(request).size()) ? true : false;
    }
     ...
     * 
     * private boolean isTimeOut(long startTime, long currTime, long maxTime){ boolean isTimeOut = false; if( (currTime-startTime) >
     * maxTime) isTimeOut = true; return isTimeOut; }
     */
    ...
    protected ModelAndView handleRequestInternal(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        String pathToUserTempStore = pathToTempStore + SEPARATOR + registration.getEmailAddress() + SEPARATOR;
        ...
        if (action != null && !action.equals("")) {
            ...
            if (!currPage.endsWith(".html")) {
                ...
                // Log.info("HTML PAGE");
                indexFile = new java.io.File("webapps" + contextPath + SEPARATOR + currPage);
            }

            // indexFile = new java.io.File("webapps" + contextPath + SEPARATOR
            // + getCurrentPage(request, action));
        } else if (isZipFile(request) && (registration.equals(file.getOwner()) || registration.isLIMSAdmin())) {
            ...
            if (filePath.startsWith("s3://")) {
                ...
                BufferedInputStream inputStream = filesUtil.getS3InputStream(filePath, buffSize, 0);
                if (inputStream == null) {
                    ...
                    inputStream = filesUtil.getS3InputStream(filePath, buffSize, 0, accessKey, secretKey);
                }
                String targetPath = ProvisionFilesUtil.createTargetDirectory(userTempFolder.getAbsolutePath(), file.getOwner()
                        .getEmailAddress());
                java.io.File f = filesUtil.copyToFile(inputStream, targetPath, buffSize, filePath);
                filePath = f.getPath();
            }
            ...
            if (filePath.startsWith("http://")) {
                ProvisionFilesUtil filesUtil = new ProvisionFilesUtil();
                int buffSize = 5000 * 1024; // 5MB
                BufferedInputStream inputStream = filesUtil.getHttpInputStream(filePath, buffSize, 0);
                String targetPath = ProvisionFilesUtil.createTargetDirectory(userTempFolder.getAbsolutePath(), file.getOwner()
                        .getEmailAddress());
                java.io.File f = filesUtil.copyToFile(inputStream, targetPath, buffSize, filePath);
                filePath = f.getPath();
            ...
            try {
                warningSize = Long.parseLong(context.getInitParameter("report.bundle.slow.display.warning.size"));
            ...
            if (sizeMeg > warningSize && isCanAborted(request)) {
                ...
                if (indexEntity != null) {
                    ...
                    // indexFile = fs.get(0);
                    // }
                    String pathIndexFile = pathToUserTempStore + indexEntity.getName();
                    Log.info("	pathToUserTempStore = " + pathIndexFile);
            ...
            if (href != null) {
                ...
                if (!href.endsWith(".html")) {
                    ...
                    Log.info("HTML PAGE");
                    indexFile = new java.io.File("webapps" + contextPath + SEPARATOR + href);
                }
            }
        }
        ...
        if (html.equals("")) {
            ...
            String page = pathToIndexPage.substring(pathToIndexPage.indexOf(root) + root.length());

            // read file to string
            html = FileUtils.readFileToString(indexFile);
            int startPosBody = html.indexOf("<body>");
            int endPosBody = html.lastIndexOf("</body>");
            if (startPosBody != -1 && endPosBody != -1 && startPosBody < endPosBody) {
                html = html.substring(startPosBody + "<body>".length(), html.lastIndexOf("</body>"));
            }
        ...
        modelAndView.addObject("pathToIndexPage", ModelUtil.forJSON(pathToIndexPage));
    ...
    private String changeAttr(String html, String root, String page, String tagName, String attrName) {
        int startPos = 0;
        int endPos = 0;
        do {
                ...
                if (tagName.equals(name)/* tag.indexOf("a") > -1 */) {
                    ...
                    try {
                        if (htmlTagValidator.validate(tag)) {
                            ...
                            html = html.substring(0, startPos) + tag + html.substring(endPos);
                        } else {
    ...
    private String changeTag(String tag, String tagName, String attrName, String root, String page) throws Exception {
        ...
        do {
            // 2. get Attr name
            Log.info("startAttrName = " + startAttrName + "; endArrtName = " + endAttrName);
            ...
            if (currAttrName.equals(attrName)) {
                ...
                tag = tag.substring(0, startImgPath + 1) + newPathToImg + tag.substring(endImgPath);
                Log.info("NEW TAG = " + tag);

                // return tag;
                endImgPath = endImgPath + (newPathToImg.length() - oldPath.length());
    ...
    /*
     * private String changeAttr(String html, String root, String page, String tagName, String attrName){ int startPos = 0; int endPos = 0;
     * do{ startPos = html.indexOf("<", startPos); endPos = html.indexOf(">", startPos) + 1; if(startPos > -1 && endPos > -1) { String tag =
     * html.substring(startPos, endPos); String name = ""; StringTokenizer st = new StringTokenizer(tag, "< "); if (st.hasMoreTokens()) {
     * name = st.nextToken(); } if(tagName.equals(name)){ int startAttrPos = html.indexOf(attrName, startPos); int startImgPath =
     * html.indexOf("\"", startAttrPos); int endImgPath = html.indexOf("\"", startImgPath + 1);
     * 
     * String oldPath = html.substring(startImgPath + 1, endImgPath); String newPathToImg = change(root, page, oldPath); html =
     * html.substring(0, startImgPath + 1) + newPathToImg + html.substring(endImgPath); } startPos++; } }while(startPos > -1); return html;
     * }
     * 
     * private String change(String root, String index, String img){ index = index.substring(0, index.lastIndexOf("/")); String path = "";
     * String upDir = "../";
     * 
     * int startPos = 0;
     * 
     * int countUpDir = 0;
     * 
     * do{ startPos = img.indexOf(upDir, startPos); if(startPos>-1) { countUpDir++; } startPos++; }while(startPos > 0);
     * 
     * img = img.replace(upDir, "");
     * 
     * while(countUpDir > 0){ int lastPos = index.lastIndexOf("/"); if(lastPos > -1) index = index.substring(0, lastPos); else{
     * System.err.println("Bad path"); return img; } countUpDir--; } path = root + index + "/" +img; return path; }
     */
    ...
    private String getHtmlListEntity(Zip64File zipFile) {
        ...
        String lines = "";
        for (FileEntry entity : listEntity) {
            lines = lines + "<tr>" + "<td>" + entity.getSize() + "</td>" + "<td>" + convertDate(entity.getTimestamp(), "MM dd yy")
                    + "</td>" + "<td>" + convertDate(entity.getTimestamp(), "HH mm") + "</td>" + "<td>" + entity.getName() + "</td></tr>";
        }
