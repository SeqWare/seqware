delete from processing WHERE processing_id IN (
	select processing_id from processing WHERE workflow_run_id IS NULL AND ancestor_workflow_run_id IS NULL 
	...
	EXCEPT select processing_id from processing_studies);
