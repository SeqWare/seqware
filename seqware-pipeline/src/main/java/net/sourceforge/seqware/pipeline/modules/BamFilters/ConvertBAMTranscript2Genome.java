...
public class ConvertBAMTranscript2Genome extends Module {
  ...
  protected OptionParser getOptionParser() {
    ...
    parser.accepts("outfile", "Output BAM file with reads mapped to genomic coordinates; <outfile>.bai is also generated.").withRequiredArg();
    ...
    parser.accepts("SQheader", "data file containing @SQ records for genome in SAM format").withRequiredArg();
    parser.accepts("perl", "Path to perl").withRequiredArg();
    parser.accepts("script", "Path to perl script: sw_module_ConvertBAMTranscript2Genome.pl").withRequiredArg();
  ...
  public ReturnValue init() {
    ...
    ret.setAlgorithm("ConvertBAMTranscript2Genome");
    ret.setDescription("Translate reads from transcript coordinates to genomic coordinates.");
  ...
  public ReturnValue do_verify_parameters() {
    ...
    for (String option : new String[] {
        "infile", "outfile", "TranscriptDB", "SQheader", "perl", "script", "java", "PicardConvert", "PicardSort", "samtools"
      }) {
  ...
  public ReturnValue do_verify_input() {
    ...
    // Does SQheader exist & is it readable?
    if (FileTools.fileExistsAndReadable(new File((String) options.valueOf("SQheader"))).getExitStatus() != ReturnValue.SUCCESS) {
      ret.setExitStatus(ReturnValue.FILENOTREADABLE);
      ret.setStderr("Required data file (SQheader) is not readable");
    }
    ...
    // Does 'script' (sw_module_ConvertBAMTranscript2Genome.pl) exist?
  ...
  public ReturnValue do_run() {
    ...
    String output1 = (String)options.valueOf("outfile");
    ...
    cmd.append(options.valueOf("outfile") + " " + options.valueOf("TranscriptDB") + " " + options.valueOf("SQheader") + " ");
    cmd.append(options.valueOf("java") + " " + options.valueOf("PicardConvert")+ " " + options.valueOf("PicardSort"));
    cmd.append(" " + options.valueOf("samtools") + " " + tempDir.getAbsolutePath());
    
    RunTools.runCommand( new String[] { "bash", "-c", cmd.toString() } );

    // record the file output
    FileMetadata fm1 = new FileMetadata();
    fm1.setMetaType("application/bam");
    fm1.setFilePath(output1);
    fm1.setType("ConvertT2G-BAM-output");
    fm1.setDescription("BAM file output of ConvertBAMTranscript2Genome module.");
