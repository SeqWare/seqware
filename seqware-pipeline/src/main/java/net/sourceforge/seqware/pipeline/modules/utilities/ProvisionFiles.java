...
public class ProvisionFiles extends Module {
    ...
    // S3 specific options
    private final int s3ConnectionTimeout = ClientConfiguration.DEFAULT_SOCKET_TIMEOUT;
    private final int s3MaxConnections = ClientConfiguration.DEFAULT_MAX_CONNECTIONS;
    private final int s3MaxErrorRetry = PredefinedRetryPolicies.DEFAULT_MAX_ERROR_RETRY;
    private final int s3SocketTimeout = ClientConfiguration.DEFAULT_SOCKET_TIMEOUT;
    private ArgumentAcceptingOptionSpec<String> annotationFileSpec;
    private ArgumentAcceptingOptionSpec<String> inputFileSpec;
    private ArgumentAcceptingOptionSpec<String> inputFileMetadataSpec;
    private ArgumentAcceptingOptionSpec<String> outputFileSpec;
    private OptionSpecBuilder skipCopySpec;
    private ArgumentAcceptingOptionSpec<String> outputDirSpec;
    private OptionSpecBuilder recursiveSpec;
    private OptionSpecBuilder forceCopySpec;
    private OptionSpecBuilder skipRecordFileSpec;
    ...
    @Override protected OptionParser getOptionParser() {
        ...
        this.inputFileSpec = parser.acceptsAll(Arrays.asList("input-file", "i"),
        ...
        this.inputFileMetadataSpec = parser.acceptsAll(Arrays.asList("input-file-metadata", "im"),
                "Required: use this or --input-file, this is the input file, multiple should be specified seperately")
                .availableUnless(inputFileSpec).withRequiredArg().describedAs("a '::' delimited list of type, meta_type, and file_path.");
        ...
        parser.acceptsAll(Arrays.asList("encrypt-key-from-settings", "ekfs"),
        ...
        parser.acceptsAll(Arrays.asList("decrypt-key", "d"),
                ...
                .describedAs("cryptographic DESede key in Base64 encoded text");
        parser.acceptsAll(Arrays.asList("decrypt-key-from-settings", "dkfs"),
        ...
        this.outputDirSpec = parser.acceptsAll(Arrays.asList("output-dir", "o"), "Required: output file location").withRequiredArg()
                .describedAs("output directory path");
        this.outputFileSpec = parser.acceptsAll(Arrays.asList("output-file", "of"),
                ...
                .availableUnless(outputDirSpec).withRequiredArg().describedAs("output file path");
        this.recursiveSpec = parser.acceptsAll(Arrays.asList("recursive", "r"),
        ...
        this.forceCopySpec = parser.accepts("force-copy",
        ...
        parser.accepts("s3-connection-timeout",
        ...
        parser.accepts("s3-max-error-retries",
        ...
        parser.accepts("s3-max-socket-timeout",
        ...
        // SEQWARE-1608
        this.skipRecordFileSpec = parser
                .accepts("skip-record-file", "Optional: If specified, do not record new entries in the file table.");

        this.skipCopySpec = parser.accepts("skip-copy",
                "Optional: If specified, do not copy, the workflow has already copied the file to the output location. "
                        + "This is just used to record the file's existence. Does not work with 'recursive.'")
                .availableUnless(recursiveSpec).availableUnless(forceCopySpec);
    ...
    public ReturnValue do_verify_parameters() {
        ...
        if (!options.has(inputFileSpec) && !options.has(inputFileMetadataSpec)) {
        ...
        /*
         * for (String requiredOption : new String[] { outputDirSpec }) { if (!options.has(requiredOption)) {
        ...
        if (!options.has(outputFileSpec) && !options.has(outputDirSpec)) {
        ...
        if (options.has(outputFileSpec) && !(options.has(inputFileSpec) || options.has(inputFileMetadataSpec))) {
        ...
        if (options.has(outputFileSpec)
                && (options.has(inputFileSpec) || options.has(inputFileMetadataSpec))
                && (options.valuesOf(outputFileSpec).size() > 1 || options.valuesOf(inputFileSpec).size() > 1 || options.valuesOf(
                        inputFileMetadataSpec).size() > 1)) {
    ...
    public ReturnValue do_verify_input() {
        ...
        for (String input : inputs) {
            ...
            if (new File(input).isDirectory() && !options.has(recursiveSpec)) {
        ...
        if (options.has(outputDirSpec) && !options.valueOf(outputDirSpec).startsWith("s3://")
                && !options.valueOf(outputDirSpec).startsWith("http://")
                && !options.valueOf(outputDirSpec).startsWith("https://")) {
            File output = new File(options.valueOf(outputDirSpec));
            ...
            if (FileTools.dirPathExistsAndWritable(output).getExitStatus() != ReturnValue.SUCCESS) {
                ...
                ret.setStderr("Can't write to output directory " + options.valueOf(outputDirSpec));
                return ret;
            }
        }

        if (options.has(outputFileSpec) && !options.valueOf(outputFileSpec).startsWith("s3://")
                && !options.valueOf(outputFileSpec).startsWith("http://")
                && !options.valueOf(outputFileSpec).startsWith("https://")) {

            String directory = FileTools.getFilePath(options.valueOf(outputFileSpec));
            ...
            if (FileTools.dirPathExistsAndWritable(output).getExitStatus() != ReturnValue.SUCCESS) {
                ret.setExitStatus(ReturnValue.DIRECTORYNOTWRITABLE);
                ret.setStderr("Can't write to output directory for file " + options.valueOf(outputFileSpec));
                return ret;
            }
        }
    ...
    public ReturnValue do_run() {
        ...
        try {
            ...
            if (options.has(inputFileMetadataSpec)) {
                List<String> metaInputs = options.valuesOf(inputFileMetadataSpec);
                List<String> outputFiles = options.valuesOf(outputFileSpec);
                ArrayList<String> newArray = new ArrayList<>();
                if (inputs != null && inputs.size() > 0) {
                    newArray.addAll(inputs);
                }
                ...
                if (!input.startsWith("http") && !input.startsWith("s3") && new File(input).isDirectory()) {
                    if (options.has(recursiveSpec)) {
                        ReturnValue currRet = recursivelyCopyDir(new File(input).getAbsolutePath(), new File(input).list(),
                                options.valueOf(outputDirSpec), fileArray);
            ...
            if (options.has(skipRecordFileSpec)) {
                fileArray.clear();
            }
     ...
     * </p>
     *
     * @param input          original path to the file, a file path if local, otherwise a http path or s3 path
     * @param output         path to the destination, a file path if local, otherwise a http path or s3 path
     * @param skipIfMissing  skip missing files with only a warning
     * @param fileArray      metadata about the file (files) to be copied
     * @param fullOutputPath the full output path overrides any concatenation of variables
     * @param metadata       the specific filemetadata for this file, if there is clearly one
     * @return a boolean.
     */
    private boolean provisionFile(String input, String output, boolean skipIfMissing, ArrayList<FileMetadata> fileArray,
     ...
     *
     * @param output         where the file should be copied to
     * @param reader         a reader open to the file that needs to be copied
     * @param bufLen         passed right through to filesUtil
     * @param input          where the file is copied from
     * @param fileArray      a {@link java.util.List} object.
     * @param fullOutputPath looks like the path the output file
     * @param metadata       metadata entry for the specific file
     * @return true when the transfer succeeded
     */
    private boolean putDestination(BufferedInputStream reader, String output, int bufLen, String input, List<FileMetadata> fileArray,
            boolean fullOutputPath, FileMetadata metadata) {
        ...
        if (options.has(skipCopySpec)){
            // don't bother doing anything
            System.out.println("  Skipping copy, only logging output file");
            result = true;
        }
