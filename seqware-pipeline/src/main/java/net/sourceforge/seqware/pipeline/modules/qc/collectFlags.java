...
public class collectFlags extends Module {
  ...
  protected OptionParser getOptionParser() {
    ...
    parser.accepts("script", "Path to perl script: sw_module_extractFlag.pl").withRequiredArg();
    parser.accepts("infile", "flowcell.lane.srf/fastq").withRequiredArg();
    parser.accepts("outfile", "Collected flag information flowcell.lane.flagCollection.txt").withRequiredArg();
    parser.accepts("qcFilePath", "A path to all the qc files").withRequiredArg();
  ...
  public ReturnValue init() {
    ...
    ret.setAlgorithm("collectFlags");
    ret.setDescription("Collecting available qc flags");
  ...
  public ReturnValue do_verify_parameters() {
    ...
    for (String option : new String[] {
       "infile", "outfile","qcFilePath", "perl", "script"
      }) {
  ...
  public ReturnValue do_verify_input() {
    ...
    // Is output file path writable?
    File output2 = new File((String) options.valueOf("qcFilePath"));
    ...
    if (FileTools.fileExistsAndReadable(new File((String) options.valueOf("script"))).getExitStatus() != ReturnValue.SUCCESS) {
      ret.setExitStatus(ReturnValue.FILENOTREADABLE);
      ret.setStderr("Could not find sw_module_extractFlag.pl at "+(String)options.valueOf("script"));
    }
...
public ReturnValue do_run() {
    ...
    cmd.append(options.valueOf("infile") + " " + options.valueOf("qcFilePath") + " " ); 
    ...
    fm.setMetaType("text/collectFlags");
    fm.setFilePath(output);
    fm.setType("Collecting qc flags");
    fm.setDescription("Text file output of qc/collectFlags module.");
    ret.getFiles().add(fm);
