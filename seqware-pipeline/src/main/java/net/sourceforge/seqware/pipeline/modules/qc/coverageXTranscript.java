...
public class coverageXTranscript extends Module {
  ...
  protected OptionParser getOptionParser() {
    ...
    parser.accepts("bamfile", "aligned bam to human transcript database").withRequiredArg();
    parser.accepts("outfile",  "two column no header column 1 is relative position in transcript [0-100] and column 2 is counts").withRequiredArg();
    parser.accepts("mapfile", "~/svnroot/seqware-complete/trunk/seqware-pipeline/data/annotation_reference/hg19_transcripts.hg19.20091027.trmap").withRequiredArg();
    parser.accepts("samtoolPath", "/usr/bin/samtools").withRequiredArg();
    parser.accepts("perl", "Path to perl").withRequiredArg();
    parser.accepts("script", "Path to perl script: sw_module_CoverageAcrossTranscript.pl").withRequiredArg();
    parser.accepts("Rcom", "Path to R, /urs/bin/R").withRequiredArg();
    parser.accepts("Rscript", "Path to R: /home/jyli/svnroot/seqware/trunk/seqware-pipeline/R/plots_TRcov.R").withRequiredArg();
    parser.accepts("coveragePlotFile",  "output figure file").withRequiredArg();
    return (parser);
  }
  ...
  public ReturnValue init() {
    ...
    ret.setAlgorithm("ConverageXTranscript");
    ret.setDescription("Getting relative coverage cross transcript plot");
  ...
  public ReturnValue do_verify_parameters() {
    ...
    for (String option : new String[] {
        "bamfile", "mapfile", "outfile", "samtoolPath","perl", "script",  "Rcom", "Rscript", "coveragePlotFile"
      }) {
  ...
  public ReturnValue do_verify_input() {
    ...
    // Does samtool path exit?  
    if (FileTools.fileExistsAndReadable(new File((String) options.valueOf("samtoolPath"))).getExitStatus() != ReturnValue.SUCCESS) {
        ret.setExitStatus(ReturnValue.FILENOTREADABLE);
        ret.setStderr("Path of " + (String)options.valueOf("samtoolPath") + " not readable");
      }
 
    
    
    // Does 'mapfile' exist & is it readable?
    if (FileTools.fileExistsAndReadable(new File((String) options.valueOf("mapfile"))).getExitStatus() != ReturnValue.SUCCESS) {
      ret.setExitStatus(ReturnValue.FILENOTREADABLE);
      ret.setStderr("Genome sequence key file " + (String)options.valueOf("mapfile") + " is not readable");
    }
    ...
    // Is output file path writable?
    File outplot = new File((String) options.valueOf("coveragePlotFile"));
...
public ReturnValue do_run() {
    ...
    String outplot = (String)options.valueOf("coveragePlotFile");
    StringBuffer cmd = new StringBuffer();
    cmd.append(options.valueOf("perl") + " " + options.valueOf("script") + " " + options.valueOf("bamfile") + " ");
    cmd.append(options.valueOf("outfile") + " " + options.valueOf("mapfile") + " "+ options.valueOf("samtoolPath") + " ");
    cmd.append( tempDir.getAbsolutePath()+ " " + options.valueOf("Rcom") + " " + options.valueOf("Rscript")+ " "+ options.valueOf("coveragePlotFile") ); 
    RunTools.runCommand( new String[] { "bash", "-c", cmd.toString() } );
    
      
    // record the file output
    FileMetadata fm = new FileMetadata();
    fm.setMetaType("text/coverageXTranscript");
    fm.setFilePath(output);
    fm.setType("Getting relative coverage cross transcript informationt");
    fm.setDescription("Text file output of qc/coverageXTranscript module.");
    ret.getFiles().add(fm);
    
       
    //Need to fix here for plot in R, jyl 
    FileMetadata fm2 = new FileMetadata();
    fm2.setMetaType("png/coverageXTranscript plot");
    fm2.setFilePath(outplot);
    fm2.setType("Plotting relative coverage cross transcript informationt");
    fm2.setDescription("Coverage plot file/coverageXTranscript module.");
