...
public class Adag {
    ...
    /**
     * Adag object serialize to <adag> element and all its sub objects serialize to sub element
     * 
     * @return
     */
    ...
    private void parseWorkflow(AbstractWorkflowDataModel wfdm) {
        ...
        if (!wfdm.getFiles().isEmpty()) {
            ...
            for (Map.Entry<String, SqwFile> entry : wfdm.getFiles().entrySet()) {
                ...
                job.addFile(file);
                ProvisionFilesJob pjob = new ProvisionFilesJob(job, wfdm.getWorkflowBaseDir(), file, wfdm.getTags().get("seqware_version"));
            ...
            if (job.getFiles().isEmpty() == false) {
                for (SqwFile file : job.getFiles()) {
                    ...
                    if (file.isInput()) {
                        ...
                        pfjob.addFile(file);
                        ProvisionFilesJob parentPfjob = new ProvisionFilesJob(pfjob, wfdm.getWorkflowBaseDir(), file, wfdm.getTags().get(
                                "seqware_version"));
