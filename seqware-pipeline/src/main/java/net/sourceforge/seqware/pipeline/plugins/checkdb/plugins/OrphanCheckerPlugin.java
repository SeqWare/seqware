...
public class OrphanCheckerPlugin implements CheckDBPluginInterface {
    ...
    public void check(SelectQueryRunner qRunner, SortedMap<Level, Set<String>> result) throws SQLException {
        ...
        executeQuery = qRunner.executeQuery("SELECT sw_accession FROM processing WHERE processing_id NOT IN (select processing_id from processing_ius);", new ColumnListHandler<Integer>());
        if (executeQuery.size() > 0) result.get(Level.WARNING).add("Processings not referenced by any processing_ius: " + executeQuery.toString());
