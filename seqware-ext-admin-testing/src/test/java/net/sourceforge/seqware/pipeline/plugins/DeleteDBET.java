...
public class DeleteDBET {
    ...
    public void testInvalidParameters() throws IOException {
        ...
        listCommand = "-p net.sourceforge.seqware.pipeline.plugins.deletion.DeletionDB "
                + "-- --r 1000000";
    ...
    public void testNormalWorkflowRunDelete() throws IOException{
        ...
        listCommand = "-p net.sourceforge.seqware.pipeline.plugins.deletion.DeletionDB "
                + "-- --r 6698 -k " + keyFile.getAbsolutePath();
    ...
    public void testNormalLaneTargetWorkflowRunDelete() throws IOException{
        ...
        Assert.assertTrue("key file is empty", keyFile.exists());
        listCommand = "-p net.sourceforge.seqware.pipeline.plugins.deletion.DeletionDB "
                + "-- --r 4764 -k " + keyFile.getAbsolutePath();
        ITUtility.runSeqWareJar(listCommand, ReturnValue.SUCCESS, null);
        // double check that the workflow run has actually been deleted
        Object[] runQuery = dbCreator.runQuery(new ArrayHandler(), "SELECT * FROM workflow_run WHERE sw_accession=6683 OR sw_accession = 6684");
        Assert.assertTrue("workflow run not deleted", runQuery == null);
        // reset database when testing successful deletes
        ExtendedTestDatabaseCreator.resetDatabaseWithUsers();
    }
    ...
    public void testNormalSequencerRunTargetWorkflowRunDelete() throws IOException{
        ...
        Assert.assertTrue("key file is empty", keyFile.exists());
        listCommand = "-p net.sourceforge.seqware.pipeline.plugins.deletion.DeletionDB "
                + "-- --r 4715 -k " + keyFile.getAbsolutePath();
        ITUtility.runSeqWareJar(listCommand, ReturnValue.SUCCESS, null);
        // double check that the workflow run has actually been deleted
        Object[] runQuery = dbCreator.runQuery(new ArrayHandler(), "SELECT * FROM workflow_run WHERE sw_accession=872 OR sw_accession = 882 OR sw_accession = 5657 OR sw_accession = 6683 OR sw_accession = 6684");
        Assert.assertTrue("workflow runs not deleted", runQuery == null);
        runQuery = dbCreator.runQuery(new ArrayHandler(), "SELECT * FROM file WHERE sw_accession=881 OR sw_accession = 1963 OR sw_accession = 1978 OR sw_accession = 2139 OR sw_accession = 2160");
    ...
    public void checkThatCascadeIsDirectedToChildWorkflowRuns() throws IOException {
        ...
        Assert.assertTrue("key file is empty", keyFile.exists());
        listCommand = "-p net.sourceforge.seqware.pipeline.plugins.deletion.DeletionDB "
                + "-- --r 872 -k " + keyFile.getAbsolutePath();
        ITUtility.runSeqWareJar(listCommand, ReturnValue.SUCCESS, null);
        // double check that the workflow run has actually been deleted
        Object[] runQuery = dbCreator.runQuery(new ArrayHandler(), "SELECT * FROM workflow_run WHERE sw_accession=872 OR sw_accession = 882 OR sw_accession = 5657");
        Assert.assertTrue("workflow runs not deleted", runQuery == null);
    ...
    public void testNormalWorkflowRunDeleteWithExplicitFile() throws IOException {
        ...
        listCommand = "-p net.sourceforge.seqware.pipeline.plugins.deletion.DeletionDB "
                + "-- --r 6691 -k " + keyFile.getAbsolutePath();
        ITUtility.runSeqWareJar(listCommand, ReturnValue.SUCCESS, null);
        // double check that the workflow run has actually been deleted
