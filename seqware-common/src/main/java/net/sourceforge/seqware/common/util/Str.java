...
public class Str {
  ...
  /**
   * Returns a String suitable to be used in multiple contexts, e.g., as part of
   * a filename. The returned string will match [a-zA-Z0-9_-]+
   * 
   * Any characters outside this range will be replaced with an underscore, and
   * multiple underscores will be collapsed down to a single underscore.
   * 
   * @param name
  ...
  /**
   * Throws an exception if the string does not match
   * [a-zA-Z0-9_-]+, otherwise a no-op.
   * 
   * @param name
   *          the string to check
   * @throws UnsafeName if the name is not safe
   */
