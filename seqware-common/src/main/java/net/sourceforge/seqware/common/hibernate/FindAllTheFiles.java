...
public class FindAllTheFiles {
    ...
    /**
     * When true, we report only leafs that are nodes. When false, we report all leafs regardless of whether they are nodes.
     */
    private boolean requireFiles = true;

    /**
     * When true, we also crawl and report input files for input files. This could be quite resource intensive in this naive first approach.
     */
    ...
    private void parseProcessingsFromStack(IUS ius, Lane lane, Set<Processing> currentProcessings) {
        ...
        while (!processingStack.isEmpty()) {
            ...
            if (!usefulProcessings.contains(processing.getProcessingId()) && !unUsefulProcessings.contains(processing.getProcessingId())) {
                processingStack.addAll(processing.getChildren());
                if (processing.getFiles() != null && processing.getFiles().size() > 0) {
                    logger.debug("parseProcessingsFromStack adding " + processing.getAlgorithm() + ":" + processing.getSwAccession());
    ...
    private List<ReturnValue> printFileInfo(Experiment e, Sample sample, Study study, IUS ius, Lane lane, SequencerRun sequencerRun,
            Processing processing, WorkflowRun workflowRun, Workflow workflow, File file) {
    ...
    private ReturnValue createReturnValue(Sample sample, Processing processing, Study study, Experiment e, IUS ius, Lane lane,
            SequencerRun sequencerRun, WorkflowRun workflowRun, Workflow workflow) {
    ...
    /**
     * <p>
     * Getter for the field <code>returnValues</code>.
     * </p>
     * 
     * @return a {@link java.util.List} object.
     */
    ...
    /**
     * <p>
     * Setter for the field <code>requireFiles</code>.
     * </p>
     * 
     * @param requireFiles
     *            a boolean.
    ...
    public static void printTSVFile(Writer writer, boolean showStatus, List<ReturnValue> returnValues, String studyName,
            boolean reportInputFiles) throws IOException {
