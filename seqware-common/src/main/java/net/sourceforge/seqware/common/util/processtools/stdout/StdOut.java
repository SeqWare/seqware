...
public class StdOut {
    ...
    /**
     * <p>
     * stdout2fileEncodedStripQualPrimer.
     * </p>
     * 
     * @param p
    ...
    public static long stdout2fileEncodedStripQualPrimer(Process p, ArrayList<File> files, int linesPerStripe) throws IOException {
        ...
        if (numFiles <= 0) throw new IOException("Must specify atleast one output file");
     ...
     *             if any.
     */
    public static long stdout2file(Process p, File file, boolean binary) throws IOException {
        if (binary) {
     ...
     *             if any.
     */
    public static long stdout2file(Process p, ArrayList<File> files, boolean binary, int stripeSize) throws IOException {
        if (binary) {
     ...
     *             if any.
     */
    public static long stdout2fileEncoded(Process p, ArrayList<File> files, int linesPerStripe) throws IOException {
        long linesProcessed = 0; // Has to be a long, or else it will wrap on large files with more than 2.1 billion lines
        int numFiles = files.size();
        BufferedWriter currentFile = null;
        BufferedReader stdout = new BufferedReader(new InputStreamReader(p.getInputStream()));
     ...
     * @param files
     *            a {@link java.util.ArrayList} object.
     * @param bytesPerStripe
     *            a int.
     * @return a int.
     */
    public static int stdout2fileBinary(Process p, ArrayList<File> files, int bytesPerStripe) {
        // FIXME: Needs to be implements
        throw new UnsupportedOperationException("stdout2fileBinary has not yet been implemented");
    }
}
