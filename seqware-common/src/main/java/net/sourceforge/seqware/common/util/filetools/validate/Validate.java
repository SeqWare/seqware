...
public class Validate {
     ...
     */
    public static ReturnValue Fastq(File file, String platform, String aligner, int ends) {
            ...
            while ((line = ln.readLine()) != null) {
                ...
                if (lineCount % 4 == 1) {
                    ...
                    if (platform.startsWith("solid")) {
                        ...
                        for (int i = 1; i < line.length(); i++) {
                            if (!(line.charAt(i) == '0' || line.charAt(i) == '1' || line.charAt(i) == '2' || line.charAt(i) == '3'
                                    || line.charAt(i) == 'N' || line.charAt(i) == 'n' || line.charAt(i) == '.')) {
                        ...
                        for (int i = 0; i < line.length(); i++) {
                            if (!(line.charAt(i) == 'A' || line.charAt(i) == 'a' || line.charAt(i) == 'T' || line.charAt(i) == 't'
                                    || line.charAt(i) == 'G' || line.charAt(i) == 'g' || line.charAt(i) == 'C' || line.charAt(i) == 'c'
                                    || line.charAt(i) == 'N' || line.charAt(i) == 'n' || line.charAt(i) == '.')) {
                    ...
                    if (line.length() != readLength)
                        error = "Expected quality line at line " + lineCount + " of length " + readLength + " but found:" + line
                                + System.getProperty("line.separator") + " of length " + line.length();
                    ...
                    for (int i = 0; i < line.length(); i++) {
                        if (line.charAt(i) < 33 || line.charAt(i) > 126) {
                            error = "Expected quality line at line " + lineCount
                                    + " composed of valid phred score (chars between ASCII 33 and 126), but found invalid score " + line
                                    + System.getProperty("line.separator");
                        }
                    }
                }
        ...
        if (aligner.compareTo("bfast") == 0) {
            if ((lineCount % (4 * ends)) != 0) {
                return new ReturnValue(null, "Valid Fastq for bfast must have multiple of 4*number_of_ends lines. In this case, " + ends
                        + " ends requires a multiple of " + ends * 4 + " but this file has " + lineCount, ReturnValue.INVALIDFILE);
            }
        } else {
            if (lineCount % 4 != 0) {
                return new ReturnValue(null, "Valid Fastq must have multiple of 4 lines, but this one has " + lineCount,
                        ReturnValue.INVALIDFILE);
            }
        }
