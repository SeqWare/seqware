...
import com.github.seqware.impl.StorageInterface;
 ...
 * HBase. This allows the back-end to perform two kinds of queries, using only the 
 * primary key portion to retrieve all the versions of a particular version chain
 * or using both the primary key and the timestamp for direct access to one 
 * specific version. HBase also allows us to quickly retrieve the latest version 
...
public class SGID implements Serializable, KryoSerializable {
     ...
     * unique ID. This doesn't need to be stored in the HBase back-end, it is done for us
     */
    private Date backendTimestamp;
    
    /**
     * Create a SGID for front-end purposes only
     */
    public SGID() {
    ...
    /**
     * Back-end constructor, create a half functional SGID
     */
     ...
     */
    public SGID(long mostSig, long leastSig, long timestamp) {
        uuid = new UUID(mostSig, leastSig);
    ...
    public boolean equals(Object other) {
        if (other instanceof SGID){
            ...
            if (!uidEq) {return false;}
            if (this.backendTimestamp == null && a.backendTimestamp == null){
                return true;
            }
            if (this.backendTimestamp != null && a.backendTimestamp != null){
    ...
    @Override
    public int hashCode() {
        return uuid.hashCode() + (backendTimestamp == null ? 0 : backendTimestamp.hashCode());
    }

    @Override
    public String toString() {
        return uuid.toString() + StorageInterface.separator + (backendTimestamp == null ? 0 : backendTimestamp.getTime());
    }
